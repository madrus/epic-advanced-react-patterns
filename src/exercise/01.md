...
NOTE: __[dequal](https://www.npmjs.com/package/dequal)__ - A tiny (304B to 489B) utility to check for deep equality

## Takeaways

### Why Context Module Functions?

We can move all the complex multi-step logic of a state update into an separate function that accepts both `dispatch` function and any arguments that `dispatch` may need. In this way, we will make our main code more readable.

### KCD about Context Module Functions

One of the advantages of this pattern is that we can take all our state update logic and put it into a separate function, and then also pass the things that are required. Each of our state updates is going to require the `dispatch` function from our context.

The benefit of doing things this way is that when we have multiple dispatches that we're going to be calling. If we just leave that up to the user of our context, it's possible that they might miss a dispatch call. It's better to pass that user dispatch to this `context module function` so it can ensure that we're calling the dispatch steps in the right order.

A common thing that people will do instead is they'll put the `updateUser` inside of their consuming hook or maybe even inside of the value, but then you have to worry about memorizing a whole bunch of stuff to make sure that you can use these functions inside of a `useEffect` dependency list or a `useCallback` dependency list. That can have a spidering effect across your entire code base.

In addition to that, it also means that you can't code-split or tree-shake any of these `context module functions`. It also hurts your ability to lazily load some of these helper functions as well.

In my codebases and in codebases that I've observed at Facebook, this is the typical pattern, where they will take the `state` and also expose the `dispatch` within context. Then that `dispatch` function will be used directly by consumers.

In situations where you have multiple dispatch calls that you need to make for asynchronous UI updates, you can make a `context module function` that accepts that dispatch, as well as anything else that's needed. This function will call dispatch correctly.
