...
Moving the local state of the previous `Toggle` component into an external `ToggleContext` is an exciting feature. Our `Toggle` component has become a `ToggleContext` provider but we don't need to create a `useToggle` hook as we don't need to expose our context to the outside world. We will use it only inside the children of the `Toggle` component = `ToggleContext` provider. Therefore, these children components don't need `on` and `toggle`  to be passed as arguments.
