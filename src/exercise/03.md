...
Moving the local state of the previous `Toggle` component into an external `ToggleContext` is an exciting feature. Our `Toggle` component has become a `ToggleContext` provider. We then create a `useToggle` hook but we don't need to validate if it is defined. The nature of our combined component is that its children have only sense if used inside the `Toggle` parent as the `ToggleContext` provider. Consequently, these children components don't need `on` and `toggle`  to be passed as arguments.
