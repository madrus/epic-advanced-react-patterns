...
The idea of the __Inversion of Control__ here is when we give the devs the possibility to hook into every state update of our _default reducer_ to make changes to it instead of accepting requests for change and try to accomodate those in our own _default reducer_. It is a much more powerful way of making our hook reusable. We provide the devs with a useful hook that they can use out-of-the-box but at the same time flexibility to take responsibility to make custom changes to it if necessary.

Using this design pattern we can provide the user devs of our custom component with possibilities to extend the default state that our component uses and to extend the action handlers that can change that extended state. In a way, the user devs can now create their own customizations of our defaults without breaking the default functionality.

The only thing we have to take care of is to export not only the `useToggle` hook itself but also the default `ToggleReducer` and the `ToggleAction` enumerator. Customization of our action handlers is then possible by adding extra conditions to the actions.

In the example here, the click on the toggle button is being handled by our own `toggleStateReducer`, and the `reset` is being handled by the default `toggleReducer`.
